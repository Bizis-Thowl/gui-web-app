...
export default function History(props) {
  ...
  const getGaugeValue = (value) => {
    return (
      Object.keys(label_assignment)[parseInt(averagedState * 5)] +
      " " +
      averagedState.toFixed(2) * 100 +
      "%"
    );
  };
  ...
  return (
    ...
    >
      ...
      >
        <ToggleButton value="Signale">Signals</ToggleButton>
        <ToggleButton value="Vorhersagen">Predictions</ToggleButton>
      </ToggleButtonGroup>
      <Box
          ...
          {currentWin === "features" ? (
            <Box>
              <Typography variant="h2">
                Overview of sensor values in the last {props.histLookback}{" "} days
              </Typography>
              <GeneralLinePlot
                elems={props.currentFeatures}
                timestamps={props.history.timestamps}
              />
            </Box>
          ) : (
            <Box>
              <Typography variant="h2">
                Model predictions for the last {props.histLookback}{" "} days
              </Typography>
              <Box sx={{ display: "flex", alignItems: "center" }}>
                ...
                >
                  <Typography sx={{ color: "white" }}>Average:</Typography>
                  <GaugeChart
                    id="gauge-chart1"
                    nrOfLevels={Object.entries(label_assignment).length}
